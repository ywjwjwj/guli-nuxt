                ...
                <section class="i-teach-wrap">
                  <div class="i-teach-pic">
                    <a :href="'/teacher/'+teacher.id" :title="teacher.name" target="_blank">
                      <img style="width: 25rem;height: 8.83125rem;" :src="teacher.avatar" :alt="teacher.name">
                    </a>
                  </div>
...
export default {
  ...
  //异步调用,只调用一次
  // params: 相当于this.$route.params (this.$route.params.id => params.id)
  asyncData({params,error}){
    return teacherApi.getTeacherFrontList(1,8)
      .then(response => {
        //this.data=response.data.data相当于
        return{
